...
const Playlist = () => {
  ...
  useEffect(() => {
    ...
    if (tracksData) {
      ...
      setTracks((prevTracks) => {
        ...
        return Array.from(new Set(allTracks.map((track) => track.id))).map(
          (id) => allTracks.find((track) => track.id === id)
        );
      });
    ...
    const fetchAudioFeatures = async () => {
      if (tracksData && tracksData.items) {
        ...
        setAudioFeatures((prevAudioFeatures) => {
          ...
          return Array.from(
            new Set(allAudioFeatures.map((item) => item.id))
  ...
  const tracksWithAudioFeatures = useMemo(() => {
    ...
    return tracks.map((track) => {
      const audioFeaturesObj = audioFeatures.find(
        (item) => item && item.id === track.id
      );
        ...
        <>
          ...
          <main>
            <SectionWrapper title="Playlist" breadcrumb={true}>
              <StyledDropdown active={sortValue ? "true" : undefined}>
